...
*/
	...
	fprintf(screen,"filesize: %f GB\n",filesize/1000000000.0);

	int test = open(arg[1],O_RDONLY,0);
	assert(test != -1);


	void *mmappeddata = mmap(NULL, 1279632, PROT_READ, MAP_PRIVATE | MAP_POPULATE, test, 0);
...
//	};
  ...
  read(test,&nchunk,sizeof(int));
	...
	int totalsize = ntotal * sizeof(double) + 12 * sizeof(int);

	fprintf(screen,"Total size timestep: %f - Total size file: %f\n",totalsize/1000000000.0,filesize/1000000000.0);

	int mapstep = 1;

	lseek(test,0,SEEK_SET);

	int rc = munmap(mmappeddata, totalsize*mapstep);
	assert(rc == 0);
	
	mmappeddata = mmap(NULL, totalsize*mapstep, PROT_READ, MAP_PRIVATE | MAP_POPULATE, test, 0);


//	fprintf(screen,"TEST = %d\n",mmappeddata[0]);
	...
	int maptime = step_size*(mapstep-1);
	int offset = 0; //totalsize*mapstep;
...
//			};
		...
		if (ntimestep==maptime) {
			offset += totalsize * mapstep;
 			munmap(mmappeddata, totalsize*mapstep);
//			fprintf(screen,"RC:%d\n",rc);
//			assert(rc == 0);

			mmappeddata = mmap(NULL, totalsize*mapstep, PROT_READ, MAP_PRIVATE | MAP_POPULATE, test, offset);
			maptime += step_size*mapstep;
		};
